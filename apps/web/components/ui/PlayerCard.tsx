...
export function PlayerCard({ player, action }: PlayerCardProps) {
  ...
  return (
    <Card className={`min-w-72 bg-white dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 transition-colors ${
      isArchived
      ...
      <div className='p-6 space-y-4'>
        {isArchived && (
          <div className='bg-orange-600/20 border border-orange-600/50 rounded px-3 py-2 text-sm text-orange-700 dark:text-orange-300'>
        ...
        <div className='flex items-start gap-4'>
          {/* Profile Image */}
          {profileImageUrl && (
            <div className='w-16 h-16 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-700 relative shrink-0'>
              <Image
                src={profileImageUrl}
        ...
        <div className='space-y-2 text-sm'>
          ...
          {player.highSchool && (
            <div className='flex items-center gap-2'>
              ...
              <span className='text-slate-900 dark:text-white'>{player.highSchool}</span>
            </div>
          )}
        ...
        {player.bio && (
          <p className='text-slate-700 dark:text-slate-300 text-sm line-clamp-3'>{player.bio}</p>
        )}

        <div className='pt-4 flex gap-2'>
          {isArchived ? (
            <Button
              className='flex-1 bg-slate-400 dark:bg-slate-600 cursor-not-allowed'
              disabled
            >
              Profile Unavailable
            </Button>
