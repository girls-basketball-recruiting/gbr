...
export const getAuthContext = cache(async (): Promise<AuthContext> => {
  ...
  // Poll for up to 5 seconds to handle webhook processing delay
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (usersResult.docs.length > 0) {
      dbUser = usersResult.docs[0]!
      if (attempt > 1) {
        console.log(`✅ User found in database after ${attempt} attempts`)
      }
      break
    }
    ...
    if (attempt < maxAttempts) {
      console.log(`⏳ User not found, waiting ${delayMs}ms before retry (attempt ${attempt}/${maxAttempts})`)
      await new Promise(resolve => setTimeout(resolve, delayMs))
    }
  }

  if (!dbUser) {
    throw new Error('User not found in database after waiting for webhook')
  }
